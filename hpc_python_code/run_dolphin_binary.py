...
DATA_DIR = "./data/"
OUT_DIR = "./data_dolphin_binary70/"
os.makedirs(OUT_DIR, exist_ok=True)

# File paths
SENTENCES_FILE = os.path.join(DATA_DIR, "intersectional_test_sentences_whicon_blalib.csv")
PERSONAS = {
...
def main():
    ...
    # Load Persona DataFrames
    df_personas = {k: pd.read_csv(v) for k, v in PERSONAS.items()}

    # --- 1. BASELINE RUN ---
    for t in TEMPS:
        out_file = os.path.join(OUT_DIR, f"dolphin70b_binary_baseline_T{t}_N50.csv")
    ...
    for strat in [1, 2]:
        ...
        for t_gen in TEMPS:
            out_file = os.path.join(OUT_DIR, f"dolphin8b_binary_2stage_strat{strat}_Tgen{t_gen}_Tann{T_ANN}_N50.csv")
            if os.path.exists(out_file): continue
            print(f"Running Strat {strat} Tgen={t_gen} Tann={T_ANN}...")
            
            # Fetch the 50 personas for this specific temperature
